...
(function(g) {
  ...
  const VS = {
    ...
    getDailyProgress() {
      try {
        const d = JSON.parse(localStorage.getItem('spanish-daily') || '{}');
        const today = new Date().toISOString().slice(0, 10);
    ...
    recordLessonComplete() {
      ...
      localStorage.setItem('spanish-daily', JSON.stringify({ date: today, count }));
    ...
    speak(text, lang) {
      ...
      const effectiveLang = lang || 'es-ES';
      if (ss.googleTtsKey) {
    ...
    async _speakGoogleTTS(text, lang, apiKey, gender) {
      ...
      const voices = voiceMap[lang] || voiceMap['es-ES'];
...
Return 2–4 items covering FUNDAMENTALLY different meanings only — do not list variations of the same concept as separate entries. Each item must represent a different real-world thing or use.`;
    ...
    async generateExamples(term, translation, partOfSpeech) {
      const PROMPT =
`You are a Spanish language expert. Give 2 natural example sentences for the Spanish word "${term}" used specifically as "${translation}" (${partOfSpeech}). Return ONLY a JSON array — no other text:
[{ "es": "Spanish sentence using the word.", "en": "English translation." }, { "es": "Another natural example.", "en": "English translation." }]`;
